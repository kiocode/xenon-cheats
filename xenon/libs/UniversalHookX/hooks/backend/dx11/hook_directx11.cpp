...
static bool CreateDeviceD3D11(HWND hWnd) {
    ...
    if (hr != S_OK) {
        spdlog::error("[!] D3D11CreateDeviceAndSwapChain() failed. [rv: %lu]\n", hr);
        return false;
    }
...
static void CreateRenderTarget(IDXGISwapChain* pSwapChain) {
    ...
    if (pBackBuffer) {
        ...
        desc.Format = static_cast<DXGI_FORMAT>(HookUtils::GetCorrectDXGIFormat(sd.BufferDesc.Format));
...
namespace DX11 {
    void Hook(HWND hwnd) {
        if (!CreateDeviceD3D11(GetConsoleWindow( ))) {
            spdlog::error("[!] CreateDeviceD3D11() failed.\n");
            return;
        }
        ...
        if (g_pd3dDevice) {
            ...
            if (!pIDXGIFactory) {
                spdlog::error("[!] pIDXGIFactory is NULL.\n");
                return;
            }
...
#include <Windows.h>
namespace DX11 {
    void Hook(HWND hwnd) { spdlog::info("[!] DirectX11 backend is not enabled!\n"); }
    void Unhook( ) { }
} // namespace DX11
#endif
