...
static bool CreateDeviceD3D10(HWND hWnd) {
    ...
    if (hr != S_OK) {
        spdlog::error("[!] D3D10CreateDeviceAndSwapChain() failed. [rv: %lu]\n", hr);
        return false;
    }
...
namespace DX10 {
    void Hook(HWND hwnd) {
        if (!CreateDeviceD3D10(GetConsoleWindow( ))) {
            spdlog::error("[!] CreateDeviceD3D10() failed.\n");
            return;
        }
        ...
        if (g_pd3dDevice) {
            ...
            if (!pIDXGIFactory) {
                spdlog::info("[!] pIDXGIFactory is NULL.\n");
                return;
            }
...
#include <Windows.h>
namespace DX10 {
    void Hook(HWND hwnd) { spdlog::info("[!] DirectX10 backend is not enabled!\n"); }
    void Unhook( ) { }
} // namespace DX10
#endif
