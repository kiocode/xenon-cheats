...
static bool CreateDeviceVK( ) {
    // Create Vulkan Instance
    {
        ...
        spdlog::info("[+] Vulkan: g_Instance: 0x%p\n", g_Instance);
    }
    ...
    // Select GPU
    {
        ...
        spdlog::info("[+] Vulkan: g_PhysicalDevice: 0x%p\n", g_PhysicalDevice);
    ...
    // Select graphics queue family
    {
        ...
        spdlog::info("[+] Vulkan: g_QueueFamily: %u\n", g_QueueFamily);
    }
    ...
    // Create Logical Device (with 1 queue)
    {
        ...
        spdlog::info("[+] Vulkan: g_FakeDevice: 0x%p\n", g_FakeDevice);
    }
...
namespace VK {
    void Hook(HWND hwnd) {
        if (!CreateDeviceVK( )) {
            spdlog::error("[!] CreateDeviceVK() failed.\n");
            return;
        }
        ...
        if (fnAcquireNextImageKHR) {
            ...
            // Hook
            spdlog::info("[+] Vulkan: fnAcquireNextImageKHR: 0x%p\n", fnAcquireNextImageKHR);
            spdlog::info("[+] Vulkan: fnAcquireNextImage2KHR: 0x%p\n", fnAcquireNextImage2KHR);
            spdlog::info("[+] Vulkan: fnQueuePresentKHR: 0x%p\n", fnQueuePresentKHR);
            spdlog::info("[+] Vulkan: fnCreateSwapchainKHR: 0x%p\n", fnCreateSwapchainKHR);
...
#include <Windows.h>
namespace VK {
    void Hook(HWND hwnd) { spdlog::info("[!] Vulkan backend is not enabled!\n"); }
    void Unhook( ) { }
} // namespace VK
#endif
